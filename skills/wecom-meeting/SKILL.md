...
> 被邀请人 userid 通过 `wecom-contact` 技能获取
...
> 响铃用户 userid 通过 `wecom-contact` 技能获取
...
> invitees 的 userid 通过 `wecom-contact` 技能获取
...
2. **通讯录查询**: 调用 `wecom-contact` 技能获取通讯录成员，按姓名筛选出参与者的 userid。
...
3. **通讯录查询**: 调用 `wecom-contact` 技能获取通讯录成员，按姓名筛选出王五的 userid。
...
- **通讯录查询**: 涉及参与人时，需先通过 `wecom-contact` 技能的 `get_userlist` 接口获取全量通讯录成员，再按姓名/别名本地筛选匹配出对应的 `userid`。该接口无入参，返回当前用户可见范围内的成员列表 (含 `userid`，`name`，`alias`)
